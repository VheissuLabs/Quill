...
- **Every domain model is UUID v7-keyed** (PR 0). Migrations use `$table->uuid('id')->primary()` and `foreignUuid` for references; models use `HasUuids`, which sets `$incrementing` and `$keyType` itself — never set either by hand. No UUID appears in a URL: `Organization` resolves by `slug`.
