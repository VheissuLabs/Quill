...
test('an admin can create a client held by the organization', function () {
    ...
    $result = new CreateClient($admin)->handle(toolRequest(['name' => 'Wayne Enterprises']));
...
test('a client can be held by a named team', function () {
    ...
    $result = new CreateClient($admin)->handle(toolRequest([
        'name' => 'Wayne Enterprises',
        'team' => 'Delivery',
    ]));

    expect($result)->toContain('held by the team Delivery');
    expect($organization->clients()->sole()->parent)->toBeInstanceOf(Team::class);
});
...
test('the word team in the name does not break the match', function (string $supplied) {
    ...
    Team::factory()->heldBy($organization)->create(['name' => 'Development']);
...
test('an ambiguous team name asks instead of guessing', function () {
    ...
    Team::factory()->heldBy($organization)->create(['name' => 'Design']);
    Team::factory()->heldBy($organization)->create(['name' => 'Design Ops']);
    ...
    expect($organization->clients()->count())->toBe(1);
});
...
test('a partial name matching two teams creates nothing', function () {
    ...
    Team::factory()->heldBy($organization)->create(['name' => 'Design Ops']);
    Team::factory()->heldBy($organization)->create(['name' => 'Design Research']);
...
test('an existing client is returned rather than duplicated', function () {
    ...
    Client::factory()->heldBy($organization)->create(['name' => 'Acme Title Co']);

    $result = new CreateClient($admin)->handle(toolRequest(['name' => 'acme title co']));
...
test('a rephrased name is recognised as the same client', function (string $rephrased) {
    ...
    $result = new CreateClient($admin)->handle(toolRequest(['name' => $rephrased]));
...
test('the same client name in another organization is not a duplicate', function () {
    ...
    new CreateClient($admin)->handle(toolRequest(['name' => 'Acme Title Co']));

    expect($organization->clients()->sole()->name)->toBe('Acme Title Co');
});
...
test('a client is never created in another organization', function () {
    ...
    new CreateClient($admin)->handle(toolRequest(['name' => 'Wayne Enterprises']));
...
test('a blank name asks rather than creating', function (mixed $name) {
    ...
    $result = new CreateClient($admin)->handle(toolRequest(['name' => $name]));
...
test('the tool declares no organization argument', function () {
    [, $admin] = organizationWith(OrganizationRole::Admin);

    $keys = array_keys(new CreateClient($admin)->schema(new Illuminate\JsonSchema\JsonSchemaTypeFactory));
